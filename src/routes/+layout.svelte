...
<div
	...
	class={[
		...
		alert_manager.s ? '' : 'hidden',
	]}>
	<div class="font-bold mb-1">{alert_manager.s?.title}</div>
	<div>{alert_manager.s?.description}</div>
</div>
<div class="flex h-dvh flex-col gap-8 p-8">
	{@render children()}
	<footer class="fixed right-16 bottom-12 z-10 mt-auto text-right text-sm text-muted/50">
